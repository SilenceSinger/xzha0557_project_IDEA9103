...
function preload() {
  sound = loadSound('assets/The_Great_Gig_in_the_Sky.mp3', loaded);
}
...
function setup() {
  ...
  // load the sound to respond
  fft = new p5.FFT();
  fft.setInput(sound);

  for (let i = 0; i < numRains; i++) {
    let x = random(width), y = random(height);
    rains.push(new Rain(x, y));
  }
}
...
function draw() {
  ...
  for (let i = 0; i < numRains; i++) {
    let rain = rains[i];
    let index = int(map(i, 0, numRains, 0, spectrum.length - 1));
    let amplitude = map(spectrum[index], 0, 255, 0, 1); 
    rain.update(amplitude);
    translate(0, -5);
    rain.display();
  }
}
...
// create a class to draw rains for animation
class Rain {
  constructor(x, y) {
    this.x = x;
    this.y = y;
  ...
  update(amplitude) {
    this.y = map(amplitude, 0, 1, 0, height);
  }

  display() {
    fill(255);
    noStroke();
    ellipse(this.x, this.y, 1, 20);
  }
}
